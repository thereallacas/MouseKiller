...
typedef struct lyuk{
    /**A lyuk koordinatai - integer tipus -x koordinata*/
    int x;
    /**A lyuk koordinatai - integer tipus -y koordinata*/
    int y;
    /**A lyuk sugara*/
    int r;
...
typedef struct possiblepos{
    /**A pozicio x koordinataja*/
    int x;
    /**A pozicio y koordinataja*/
    int y;
}pp;

/**Ez a struktura az egerek felbukkkanasanak SDL_Timer -ehez valo visszateresi
 * ido veletlenszeru generalasahoz szukseges. Also es felso korlatot szab a
 * rand() fuggvenynek.*/
typedef struct IDOK{
...
/**Ez a fuggveny 5 SDL-timer egyszerre valo letiltasara alkalmas. Letezese csupan az
 ...
 * @param koncolfrissit SDL_TIMERID 5. parameter
 */
  void REMOVE_TIMERS(SDL_TimerID felbukkan, SDL_TimerID eletetveszt, SDL_TimerID nehezedik, SDL_TimerID jatek_vege, SDL_TimerID idofrissit){
    SDL_TimerID tomb[]={felbukkan, eletetveszt, nehezedik, jatek_vege, idofrissit};
    int i;
    for (i=0; i<5; i++){
...
/**EVENTRESERVED3-at(egermegjelenest) idez elo. Ez a fuggveny tovabba megkapja a
...
/**EVENTRESERVED2-ot(jatekbefejezest) idez elo. Ha idaig eljutott a jatekos, akkor nyert.
 * @param ms Uint32 ido
...
/**EVENTRESERVED4-et(eletvesztest) idez elo. Ez a fuggveny teszi lehetove, hogy a
...
/**EVENTRESERVED5-ot(nehezedest) idez elo. Mit is erne a jatek nehezedes nelkul?
...
/**EVENTRESERVED7-et(ido kiirasat) idezi elo.
 * @param ms Uint32 ido
 * @param param tetszoleges parameter
 * @return ms Uint32
 */
  Uint32 idokiir(Uint32 ms, void *param){
    SDL_Event even;
    even.type = SDL_EVENT_RESERVED7;
    SDL_PushEvent(&even);
    return ms;
}
 ...
 */
  void cprajzolas(SDL_Surface *hova, SDL_Surface *kep, int point){
    ...
    TTF_KIIR(screen, JSZELESSEG+5, 10, buffer, betutipus, WHITE);
}

void timekiir(SDL_Surface *hova, int actual){
    char ido[50];
    sprintf(ido, "Remaining:%d", ((100*1000)-actual)/1000);
    boxRGBA(screen, JSZELESSEG+5, 150, JSZELESSEG+190, 180, 0,0,0,255);
    TTF_KIIR(screen, JSZELESSEG+5, 150, ido, betutipus3, WHITE);
}ticks;
...
/**Új lyukat fuz a regiek listajahoz, tomb kovetkezo elemebol vesz hozza adatokat.
 * @param head a lista kezdoelemere mutato pointert kell megadni
...
/*Timerek inicializalasa*/
    ...
    #endif // TEST*/
    eletetveszt=SDL_AddTimer(4*1000, fogyo, NULL);
    nehezedik=SDL_AddTimer(5*1000, harder, NULL);
    uj=SDL_AddTimer(3*1000, newhole, NULL);
    idofrissit=SDL_AddTimer(1, idokiir, NULL);
    ...
    while (!(*quit)&&!lose&&!end&&!enough){
        ...
        switch (ev.type) {
            case SDL_EVENT_RESERVED7:
                actual=SDL_GetTicks()-starttime;
                timekiir(screen, actual);
                SDL_UpdateRect(screen, JSZELESSEG+5, 150, 190, 30);
                break;
            ...
            case SDL_KEYDOWN:
                switch(ev.key.keysym.unicode){
                    #ifdef TEST
            /*Teszteloi cheat kod. Ha ?-t nyomunk, akkor talalat van
            *(nem tokeletesen helyezi el a halott egeret,
            *de az most mindegy*/
                    case SDLK_j:               /*LEÍRÁS KÉSŐBB*/
                    palyarajzolas(screen, head);
                        aktiv=head;
...
/*A kulonbozo elemek felszabaditasa*/
 ...
 SDL_Delay(4*1000);
    ...
    REMOVE_TIMERS(felbukkan,nehezedik,eletetveszt,jatek_vege, idofrissit);
